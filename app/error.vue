...
<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 flex items-center justify-center px-4 py-12">
    <div class="max-w-2xl w-full">
      
      <!-- Error Icon & Code -->
      <div class="text-center mb-8">
        <div class="inline-flex items-center justify-center w-24 h-24 rounded-full bg-white shadow-lg mb-6 border border-slate-100">
          <UIcon :name="errorIcon" :class="['w-12 h-12', iconColor]" />
        </div>
        
        <div class="inline-block px-4 py-1 rounded-full bg-slate-100 text-slate-600 text-sm font-mono mb-4">
          Error {{ statusCode }}
        </div>
      </div>
      ...
      <div v-if="isDev" class="mt-12 p-6 bg-red-50 border border-red-200 rounded-xl">
        ...
        <div class="space-y-3 text-xs">
          ...
          <div>
            ...
            <p class="text-red-700 mt-1 font-mono break-words">{{ error.message || 'No message' }}</p>
          </div>
          ...
          <div v-if="error.stack">
            ...
            <pre class="text-red-700 mt-2 p-3 bg-white rounded border border-red-200 overflow-x-auto text-[10px] leading-tight">{{ error.stack }}</pre>
          </div>
        </div>
      </div>
