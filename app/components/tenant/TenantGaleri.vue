...
<template>
  <section class="py-24 bg-white relative overflow-hidden">
    ...
    <div class="absolute top-1/2 right-[-20%] -translate-y-1/2 w-[50rem] h-[50rem] bg-red-50/60 rounded-full blur-[120px] pointer-events-none"></div>
    ...
    <div class="container mx-auto px-4 lg:max-w-7xl relative z-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 auto-rows-[250px] gap-4 md:gap-6 grid-flow-dense">
        ...
        >
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/30 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-6">
            
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center scale-50 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-500 delay-100">
            ...
            <div class="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500 delay-200">
              ...
              <h3 class="text-white font-bold text-lg md:text-xl leading-tight line-clamp-2">{{ foto.title }}</h3>
            </div>
          </div>
        </div>
      ...
      <div class="text-center mt-12">
         <UButton variant="outline" color="gray" class="rounded-full px-8 py-3 font-semibold bg-white hover:bg-slate-50 border-slate-200 text-slate-700 transition-all active:scale-95">
    ...
    <UModal v-model="isOpen" :ui="{ width: 'max-w-5xl', overlay: { background: 'bg-slate-900/90 backdrop-blur-sm' } }">
      <div class="relative bg-slate-900 rounded-2xl overflow-hidden shadow-2xl max-h-[90vh] flex flex-col md:flex-row">
        
        <div class="flex-grow relative bg-black flex items-center justify-center overflow-hidden group md:w-2/3">
           <img :src="selectedImage.src" :alt="selectedImage.title" class="max-w-full max-h-[85vh] object-contain" />
        </div>

        <div class="bg-white p-8 md:w-1/3 flex flex-col justify-center relative">
            <button @click="isOpen = false" class="absolute top-4 right-4 p-2 bg-slate-100 text-slate-500 rounded-full hover:bg-red-50 hover:text-red-600 transition-colors">
              <UIcon name="i-lucide-x" class="w-5 h-5" />
            </button>

            <span class="text-xs font-bold text-red-600 mb-2 uppercase tracking-wider flex items-center gap-2">
              <UIcon name="i-lucide-tag" class="w-4 h-4" /> {{ selectedImage.category }}
            </span>
            <h3 class="text-2xl font-extrabold text-slate-900 leading-tight mb-4">{{ selectedImage.title }}</h3>
            <p class="text-slate-600 leading-relaxed text-sm">
              Deskripsi detail mengenai kegiatan ini dapat ditambahkan di sini. Ini memberikan konteks lebih pada foto yang ditampilkan kepada pengunjung.
            </p>

            <div class="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between text-sm text-slate-500">
              <span class="flex items-center gap-1"><UIcon name="i-lucide-calendar" class="w-4 h-4" /> 20 Feb 2026</span>
              <span class="flex items-center gap-1"><UIcon name="i-lucide-camera" class="w-4 h-4" /> Dokumentasi KMP</span>
            </div>
        </div>
